...
const ModalGlobal: React.FC = () => {
  ...
  return (
    <div className="fixed top-0 left-0 w-100 h-100 bg-black-50 flex items-center justify-center z-9999">
      <div className="bg-white pa4 br3 shadow-3 w-90 mw6 items-center flex flex-column justify-center">
        <h2 className="f3 mb3">{title}</h2>
        <p className="f5 mb4">{message}</p>
        <button onClick={handleClose} className="bg-brand white ph3 pv2 bn btn btn-primary">
          {confirmText}
        </button>
      </div>
    </div>
